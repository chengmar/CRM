...
$reportDir = Join-Path $Workspace "outputs\fresh_install_acceptance"
New-Item -ItemType Directory -Force -Path $reportDir | Out-Null
$reportPath = Join-Path $reportDir ("fresh-install-acceptance-" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".json")
...
Invoke-Check "Required installation files" {
  $required = @(
    $envPath,
...
Invoke-Check "Fresh customer CSV schema" {
  $header = Get-Content -LiteralPath (Join-Path $Workspace "$businessDataDir\leads.csv") -TotalCount 1 -Encoding UTF8
  foreach ($field in @("company", "website", "country", "email", "score", "status")) {
    if ($header -notmatch "(^|,)" + [regex]::Escape($field) + "(,|$)") { throw "Missing CSV field: $field" }
  }
  "lead_rows_not_required=true"
}
...
Invoke-Check "Controlled outbound defaults" {
  ...
  foreach ($name in $required.Keys) {
    $actual = Get-EnvValue $envText $name
    if ($actual.ToLowerInvariant() -ne $required[$name]) { throw "$name must be $($required[$name])" }
  }
  if ($gmailPilot) {
    ...
    if ([int](Get-EnvValue $envText "EMAIL_DAILY_LIMIT" "0") -lt 1 -or
        [int](Get-EnvValue $envText "EMAIL_DAILY_LIMIT" "0") -gt 100) {
      throw "Gmail pilot daily target must be between 1 and 100."
    }
    if ([int](Get-EnvValue $envText "EMAIL_HOURLY_LIMIT" "0") -lt 1 -or
        [int](Get-EnvValue $envText "EMAIL_HOURLY_LIMIT" "0") -gt 20) {
      throw "Gmail pilot adaptive hourly ceiling must be between 1 and 20."
    }
    if ([int](Get-EnvValue $envText "EMAIL_MIN_INTERVAL_SECONDS" "0") -lt 60) {
  ...
  "email_enabled=$emailOutreachEnabled; gmail_pilot=$gmailPilot; enterprise_email=$enterpriseEmail; outbound_capability=$expectedOutboundCapability; runtime_pause_required=true; send_receive_self_test_required=$enterpriseEmail"
}
...
Invoke-Check "Agent health and safe mode" {
  $health = Invoke-RestMethod -Uri "http://127.0.0.1:18790/health" -TimeoutSec 10
  ...
  if ($health.dailyResearchEnabled) { throw "Fresh install must keep daily research disabled until an operator enables it." }
  if ($gmailPilot) {
    if (-not $health.gmailPilot.mode) { throw "Health endpoint does not report Gmail pilot mode." }
    if ($health.gmailPilot.selfTestPassed) { throw "Fresh install must require a new Gmail self-test." }
    if ($health.gmailPilot.activated) { throw "Fresh install must not pre-authorize Gmail pilot sending." }
  }
  if ($enterpriseEmail) {
  ...
  "schema=$($health.schemaVersion); feishu_connected=$($health.feishuConnected); outbound_capability=$($health.outboundEnabled); outbound_paused=$($health.outboundPaused); daily_research_enabled=$($health.dailyResearchEnabled)"
}
...
Invoke-Check "Agent readiness contract" {
  $readiness = Invoke-RestMethod -Uri "http://127.0.0.1:18790/readiness" -TimeoutSec 20
  if (-not $readiness.ok) { throw "Readiness database checks failed." }
  if ([int]$readiness.migration.currentVersion -ne $ExpectedSchemaVersion -or
      [int]$readiness.migration.latestVersion -ne $ExpectedSchemaVersion) {
    throw "Readiness expected schema $ExpectedSchemaVersion, got current=$($readiness.migration.currentVersion) latest=$($readiness.migration.latestVersion)."
  }
  if (-not $readiness.dryRunSafe) { throw "Readiness does not report a safe paused state." }
  if (-not $readiness.searchConfigured) { throw "Search provider is not configured." }
  if ($readiness.feishuBotConfigured) {
    if (-not $readiness.inquiryAlertDestinationConfigured) { throw "Feishu inquiry alert destination is not bound." }
    if (-not $readiness.bitableSchema.ok) { throw "Feishu Bitable schema is invalid." }
  }
  "search=true; bitable=$($readiness.bitableSchema.ok); gmail_pilot=$($readiness.gmailPilotConfigured)"
}

Invoke-ExternalCheck "Agent CLI database verification" {
  Push-Location (Join-Path $Workspace "agent_service")
  try { & node dist/cli.js verify-db } finally { Pop-Location }
}

$feishuEnabled = (Get-EnvValue $envText "FEISHU_BOT_ENABLED" "false").ToLowerInvariant() -eq "true"
if ($feishuEnabled) {
  Invoke-ExternalCheck "Feishu Bitable schema" {
    Push-Location (Join-Path $Workspace "agent_service")
    try { & node dist/cli.js validate-bitable } finally { Pop-Location }
  }
}
...
Invoke-ExternalCheck "SQLite restart persistence" {
  & (Join-Path $Workspace "scripts\test-agent-service-persistence.ps1") -Workspace $Workspace
}

Invoke-ExternalCheck "Initial production backup" {
  & (Join-Path $Workspace "scripts\backup-production-state.ps1") -Workspace $Workspace -Reason "fresh-install-acceptance" -KeepLatest 30
}
...
$report = [pscustomobject]@{
  ...
  clean_server_declared = [bool]$CleanServer
